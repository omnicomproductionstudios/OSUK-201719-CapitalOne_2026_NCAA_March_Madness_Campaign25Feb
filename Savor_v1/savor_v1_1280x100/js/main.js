...
function init(){
  ...
  gsap.set(".basketballCont", { x: 120, y: 100 });
  ...
  mainTL.from(redSwoop, 0.35,{alpha:0, scale:4, x:"-=150", y:"+=50", ease:'power3.out'}, _f1)
        ...
        .to(".basketballCont", 6, { x: 1280, ease: "power1.inOut" }, 0.5)
...
function expand(){
  isExpanded = true;
  cardTL.clear();
  gsap.set(".expBasketballCont", { x: 727, y: 418, scale:1.8 });
  // gsap.set("#expBasketball", { rotation: 180, transformOrigin: "50% 50%" });
  ...
  cardTL.from('.expCapOneLogo #redSwoop', 0.35,{alpha:0, scale:4, x:"-=150", y:"+=50", ease:'power3.out'}, _f1+0.2)
        ...
        .from('.expCapOneLogo #one', 0.35,{alpha:0, y:"+=15", ease:'power3.out'}, '<+0.04')
         ...
         .to(".expBasketballCont", 6, { x: 1050, ease: "power1.inOut" }, 0.5)
